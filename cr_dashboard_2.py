    ...
    if side == "North":
        ...
        st.markdown(
            ...
            + "".join(
                [
                    (
                        f"<div style='background-color:{get_station_color(s, retrieved_df, retrieved_df_totes)}; padding:40px; border:2px solid black; font-size:24px; width:200 px; height:150 px; text-align:center; display:flex; align-items:center; justify-content:center;'>{s}<br>{fetch_live_value(s, retrieved_df)}<br>{fetch_live_value(s, retrieved_df_totes)}</div>"
                        if s
